...
function makeSupabaseQuery(selectResults: any[]) {
    const insert = vi.fn(() => ({ select: vi.fn().mockResolvedValue(selectResults.shift() ?? { data: null, error: null }) }));
    const select = vi.fn(() => ({ eq: vi.fn(() => ({ single: vi.fn().mockResolvedValue(selectResults.shift() ?? { data: null, error: null }) })) }));
...
describe('POST /api/deployments', () => {
    ...
    it('returns 404 when template not found', async () => {
        const templatesTable = makeSupabaseQuery([{ data: null, error: { message: 'not found' } }]);
    ...
    it('returns 422 for invalid customization config', async () => {
        const templatesTable = makeSupabaseQuery([{ data: { id: 'tpl-1', name: 'My Template' }, error: null }]);
        ...
        const invalidConfig = {
            branding: { appName: '', primaryColor: '#fff', secondaryColor: '#fff', fontFamily: 'Inter' },
    ...
    it('creates deployment and returns 201', async () => {
        const templatesTable = makeSupabaseQuery([{ data: { id: 'tpl-1', name: 'My Template' }, error: null }]);
        const deploymentsInsertResult = { data: { id: 'dep-1', template_id: 'tpl-1', user_id: fakeUser.id, name: 'My Template', customization_config: {}, created_at: new Date().toISOString() }, error: null };
        ...
        const validConfig = {
            branding: { appName: 'App', primaryColor: '#000000', secondaryColor: '#111111', fontFamily: 'Inter' },
            features: { enableCharts: true, enableTransactionHistory: true, enableAnalytics: false, enableNotifications: false },
            stellar: { network: 'testnet', horizonUrl: 'https://horizon-testnet.stellar.org' },
        };
