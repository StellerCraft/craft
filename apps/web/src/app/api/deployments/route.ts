...
import { validateCustomizationConfig, validateStellarEndpoints } from '@/lib/customization/validate';
...
export const POST = withAuth(async (req: NextRequest, { supabase, user }) => {
    ...
    try {
        const endpointValidation = await validateStellarEndpoints(customization as any, { timeout: 3000 });
        if (!endpointValidation.valid) {
            return NextResponse.json(
                { error: 'Invalid customization endpoints', details: endpointValidation.errors },
                { status: 422 },
            );
        }
    } catch (err: any) {
        // Treat connectivity errors as transient server errors
        return NextResponse.json({ error: err?.message ?? 'Endpoint validation failed' }, { status: 500 });
    }
    ...
    if (insertErr || !inserted) {
        return NextResponse.json({ error: insertErr?.message ?? 'Failed to create deployment' }, { status: 500 });
    }
