...
/**
 * Resolve the Soroban RPC URL, preferring the user-supplied value and falling
 * back to the canonical default for the selected network.
 */
function resolveSorobanRpcUrl(stellar: StellarConfig): string {
    return stellar.sorobanRpcUrl || DEFAULT_SOROBAN_RPC_URL[stellar.network];
}
 ...
 */
export function buildStellarEnvVars(stellar: StellarConfig, family?: TemplateFamilyId): StellarEnvVars {
    ...
    if (stellar.sorobanRpcUrl || family === 'soroban-defi') {
        vars.NEXT_PUBLIC_SOROBAN_RPC_URL = resolveSorobanRpcUrl(stellar);
    }
    ...
    if (contractsSerialized !== undefined || family === 'soroban-defi' || family === 'asset-issuance') {
...
export function generateStellarConfigFile(
    ...
    const sorobanRpcUrl = resolveSorobanRpcUrl(stellar);

    const sorobanLine =
        stellar.sorobanRpcUrl || family === 'soroban-defi'
            ? `    sorobanRpcUrl: process.env.NEXT_PUBLIC_SOROBAN_RPC_URL || '${sorobanRpcUrl}',\n`
            : '';

    const assetPairsLine = usesAssetPairs(family) ? buildAssetPairsConfigLine(stellar.assetPairs) : '';
    const contractsLine = usesContractAddresses(family) ? buildContractAddressesConfigLine(stellar.contractAddresses) : '';
